...
function getDB(a, m) {
    const k = getKey(a, m);
    // Tenta cache
    if (dbMensal[k]) return dbMensal[k];
...
function renderList(id, arr, type, extra) {
    ...
    mapped.forEach((it) => {
        ...
        content.addEventListener('touchstart', (e) => { startX = e.touches[0].clientX; isDragging = true; content.classList.add('is-swiping'); }, {passive: true});
        content.addEventListener('touchmove', (e) => { if (!isDragging) return; currentX = e.touches[0].clientX; const diff = currentX - startX; if (diff > 0 && diff < 100) content.style.transform = `translateX(${diff}px)`; else if (diff < 0 && diff > -100) content.style.transform = `translateX(${diff}px)`; }, {passive: true});
        content.addEventListener('touchend', () => { isDragging = false; content.classList.remove('is-swiping'); const diff = currentX - startX; 
            if (diff > 80) { content.style.transform = `translateX(0px)`; openEdit(type, it.originalIndex); } 
            else if (diff < -80) { content.style.transform = `translateX(-100%)`; setTimeout(() => del(type, it.originalIndex), 200); } 
...
function confirmBuy() {
    ...
    if (t === 'debit') {
        ...
        // Empurra para o array correto
        db.debit.push({ 
            n: it.n, 
            v: it.v, 
            d: diaSel || 1, // Garante que tem dia
            c: c,
            o: 'Compra desejo' 
        });
        ...
        // Logica de fechamento simplificada para Wishes
        ...
        for (let i = 0; i < p; i++) {
            ...
            // Pega direto do disco para não depender do cache do mês atual
            let dF;
            try {
            ...
            dF.credit.push({ 
                n: `${it.n} (${i + 1}/${p})`, 
                v: vP, 
                d: 1, // Dia 1 padrão para parcelas
                c: c,
                o: 'Compra desejo'
            });
            
            // Salva mês específico
...
function saveQuickAdd() {
    ...
    // Recarrega
    if(targetY !== dataNav.getFullYear() || targetM !== dataNav.getMonth()) render(); // Update totais globais
    else render(); 
...
function add(tipo) {
    ...
    const v = parseFloat(document.getElementById(vId).value);
...
function updateCalendar(a, m, ld) {
    ...
    const d = getDB(a, m);
    if (!d || !d.debit || !d.credit) return; // Safety
...
function renderChart(cA, cM) { const ctx = document.getElementById('balanceChart'); if (myChart) myChart.destroy(); if(!ctx) return; const db = getDB(cA, cM); if(chartType==='bar') { myChart = new Chart(ctx, {type:'bar', data: {labels:['S','D'], datasets:[{data:[100,50]}]}}); } else { myChart = new Chart(ctx, {type:'doughnut', data:{labels:['A','B'], datasets:[{data:[30,70]}]}}); } }
function generateAdvice(v) { return ""; }
