...
/* --- DICA DA IA (SIMULAÇÃO INTELIGENTE) --- */
function generateAdvice(totalVal) {
    const metaDiaria = settings.dailyGoal || 30;

    // 1. Estimar Renda Base (Usa a renda do mês ATUAL ou o Limite do cartão como fallback)
    const currentYear = dataNav.getFullYear();
    const currentMonth = dataNav.getMonth();
    const dbAtual = getDB(currentYear, currentMonth);
    
    // Soma todas as entradas do mês atual
    let rendaBase = (dbAtual.in || []).reduce((a, b) => a + b.v, 0);
    
    // Se não tiver renda lançada ainda, usa o Limite configurado para não quebrar a conta
    if (rendaBase === 0) rendaBase = settings.limit || 2000; 

    // 2. Calcular Custo Fixo Mensal (Global)
    const custoFixo = dbGlobal.fixed.reduce((a, b) => a + b.v, 0);

    let html = `<div class='sug-list' style='max-height:250px; overflow-y:auto; font-size:0.8rem; margin-top:10px'>`;
    html += `<div style='padding:8px; background:var(--bg); border-radius:6px; margin-bottom:8px; border:1px solid var(--border)'>
                🎯 Meta: Sobra > <strong>${fmt(metaDiaria)}/dia</strong>
             </div>`;
    ...
    for (let p = 1; p <= 12; p++) {
        ...
        // Procura o PRIMEIRO mês (nos próximos 12) onde essa parcela cabe na meta
        for (let offset = 0; offset < 12; offset++) {
            // Data alvo (meses futuros)
            let targetDate = new Date(currentYear, currentMonth + offset, 1);
            let tY = targetDate.getFullYear();
            let tM = targetDate.getMonth();

            // Busca gastos JÁ comprometidos no cartão para esse mês futuro
            // (Lemos direto do localStorage para não poluir o cache de visualização atual)
            let k = getKey(tY, tM);
            let s = localStorage.getItem(k);
            let dF = s ? JSON.parse(s) : { credit: [] };
            
            let cartaoComprometido = (dF.credit || []).reduce((a, b) => a + b.v, 0);

            // Simulação: Renda - Fixos - CartãoJáExistente - NovaParcela
            let saldoLivre = rendaBase - custoFixo - cartaoComprometido - parcVal;
            
            // Divide por 30 para ver o diário
            let diarioPrevisto = saldoLivre / 30;

            if (diarioPrevisto >= metaDiaria) {
                melhorMes = targetDate;
                sobraDiariaSimulada = diarioPrevisto;
                break; // Encontrou o mês mais próximo possível
            }
        }

        // Monta a visualização da linha
        let rowColor = melhorMes ? "var(--text)" : "var(--text-muted)";
        let statusColor = melhorMes ? "var(--green)" : "var(--red)";
        let statusIcon = melhorMes ? "✅" : "🚫";
        let statusText = melhorMes 
            ? `${statusIcon} ${melhorMes.toLocaleString('pt-BR', {month:'short'}).toUpperCase()}` 
            : "Apertado";
        
        let extraInfo = melhorMes 
            ? `(Sobra: ${fmt(sobraDiariaSimulada)}/dia)` 
            : `(< ${fmt(metaDiaria)})`;

        html += `<div class="sug-item" style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px dashed var(--border); align-items:center; color:${rowColor}">
                    <div style="display:flex; flex-direction:column;">
                        <span style="font-weight:bold;">${p}x de ${fmt(parcVal)}</span>
                        <span style="font-size:0.7rem; opacity:0.7">${extraInfo}</span>
                    </div>
                    <div style="text-align:right; font-size:0.75rem; color:${statusColor}; font-weight:bold;">
                        ${statusText}
                    </div>
                 </div>`;
    }
    ...
    html += "</div>";
    return `<details style='margin-top:10px; background:var(--input-bg); padding:10px; border-radius:10px; border:1px solid var(--border)'>
                <summary style='cursor:pointer; font-weight:bold; color:var(--primary); list-style:none; display:flex; justify-content:space-between'>
                    <span>💡 IA: Análise de Parcelas</span>
                    <span>▼</span>
                </summary>
                ${html}
            </details>`;
...
function renderList(id, arr, type, extra) {
    ...
    if (!arr || arr.length === 0 && !extra) { 
        ul.innerHTML += "<div class='empty-msg' style='text-align:center; padding:15px; opacity:0.5; font-size:0.9rem'>Nada neste mês.</div>"; 
        return; 
    }
...
function add(tipo) {
    ...
    if (tipo === 'in') { nId='inNome'; vId='inVal'; dId='inDate'; cId='catIn'; oId='inObs'; } 
        ...
        const n = document.getElementById('wishNome').value; const v = parseFloat(document.getElementById('wishVal').value);
        if (n && v > 0) { dbGlobal.wishes.push({ n, v }); saveGlobal(); document.getElementById('wishNome').value = ''; document.getElementById('wishVal').value = ''; render(); } else alert("Preencha corretamente."); return;
    }
    const elN = document.getElementById(nId); const elV = document.getElementById(vId); const elObs = document.getElementById(oId); const elDate = document.getElementById(dId);
    if (elN && elV) {
        const n = elN.value; const v = parseFloat(elV.value); const o = elObs ? elObs.value : ''; let dVal = parseInt(elDate.value) || 1; 
        if (n && !isNaN(v) && v > 0) {
            if (tipo === 'credit') {
                const closingDay = getBusinessClosingDate(targetY, targetM);
                if (dVal >= closingDay) { 
            ...
            const dbRef = getDB(targetY, targetM); let tArr = (tipo==='in') ? dbRef.in : (tipo==='debit' ? dbRef.debit : dbRef.credit);
            tArr.push({ n, v, d: dVal, c: document.getElementById(cId).value, o: o });
            if (targetY !== dataNav.getFullYear() || targetM !== dataNav.getMonth()) {
               localStorage.setItem(getKey(targetY, targetM), JSON.stringify(dbRef)); render();
            } else { save(); }
...
function confirmBuy() {
    const it = dbGlobal.wishes[selectedWishIndex]; const t = document.getElementById('modalType').value; const c = document.getElementById('modalCat').value;
    let targetY = dataNav.getFullYear(); let targetM = dataNav.getMonth();
    if (t === 'debit') {
        const db = getDB(targetY, targetM); db.debit.push({ n: it.n, v: it.v, d: diaSel, c: c }); save();
    } else {
        const p = parseInt(document.getElementById('modalInstallments').value) || 1; const s = document.getElementById('modalStartMonth').value; let[y,m] = s.split('-').map(Number); m--;
        const closingDay = getBusinessClosingDate(y, m);
        if (y === targetY && m === targetM && diaSel >= closingDay) { alert("Fatura fechada. Início próximo mês."); m++; if(m>11){m=0; y++;} }
        const vP = it.v / p;
        for (let i = 0; i < p; i++) {
            let cM = m + i; let cY = y; while (cM > 11) { cM -= 12; cY++; }
            const k = getKey(cY, cM);
            const sF = localStorage.getItem(k); const dF = sF ? JSON.parse(sF) : { in: [], debit: [], credit: [], invoicePaid: 0 };
            dF.credit.push({ n: `${it.n} (${i + 1}/${p})`, v: vP, d: 1, c: c });
            if(!isSandbox) localStorage.setItem(k, JSON.stringify(dF));
        }
        render();
    }
    dbGlobal.wishes.splice(selectedWishIndex, 1); saveGlobal(); closeBuyModal(); render();
}
function del(t, i) {
    if (confirm("Apagar?")) {
        if (t === 'wishes') { dbGlobal.wishes.splice(i, 1); saveGlobal(); render(); } 
        else { getDB(dataNav.getFullYear(), dataNav.getMonth())[t].splice(i, 1); save(); }
    }
}
function copyPrev(t) {
    let pd = new Date(dataNav.getFullYear(),dataNav.getMonth() - 1,1); let pk = getKey(pd.getFullYear(), pd.getMonth()); let s = localStorage.getItem(pk); if (!s) return alert("Sem dados anteriores."); let pData = JSON.parse(s); if (!pData[t]) return; if (confirm("Copiar?")) { const cDB = getDB(dataNav.getFullYear(), dataNav.getMonth()); pData[t].forEach(it => cDB[t].push({ n: it.n, v: it.v, d: 1, c: it.c, o: it.o })); save(); }
}
function toggleSandbox() {
    if (!isSandbox) { if (!confirm("Entrar no Modo Simulação?")) return; isSandbox = true; document.body.classList.add('sandbox-active'); document.getElementById('btnSandbox').innerText = '🚪'; alert("🧪 SIMULAÇÃO ATIVA"); }
    else { if (!confirm("Sair do simulador?")) return; isSandbox = false; document.body.classList.remove('sandbox-active'); document.getElementById('btnSandbox').innerText = '🧪'; dbMensal = {}; render(); }
}
...
function checkNotificationTime() {
    ...
    if (h === 20) {
        ...
        if (localStorage.getItem('appV30_last_notif_date') !== t) { new Notification("Fechamento! 💸", { body: "Lance seus gastos do dia." }); localStorage.setItem('appV30_last_notif_date', t); }
    }
}
...
function render() {
    // ... resto do render ...
    // Certifique-se de que a parte de Wishes está assim:
    ...
    // ...
    // Para simplificar, vou colar o RENDER completo aqui dentro para garantir que você tenha a versão com generateAdvice integrado.
    try {
        ...
        ['inDate', 'debDate', 'credDate'].forEach(id => { const el = document.getElementById(id); if(el) el.value = defaultDay; });
